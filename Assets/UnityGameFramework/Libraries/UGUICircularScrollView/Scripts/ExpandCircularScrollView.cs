    ...
    public class ExpandCircularScrollView : UICircularScrollView
    {
        ...
        public override void ShowList(string numStr)
        {
            ...
            for (int k = 0; k < buttonCount; k++)
            {
                ...
                {
                    pos = m_ExpandButtonHeight * k + m_SpacingY * (k + 1);
                    pos += k > 0 ? (m_CellObjectHeight + m_SpacingY) * Mathf.CeilToInt( (float)beforeCellCount / m_Row ) : 0;
                    button.transform.GetComponent<RectTransform>().anchoredPosition = new Vector3(m_ExpandButtonX, -pos, 0);
                }
                else
                {
                    pos = m_ExpandButtonWidth * k + m_SpacingX * (k + 1);
                    pos += k > 0 ? (m_CellObjectWidth + m_SpacingX) * Mathf.CeilToInt( (float)beforeCellCount / m_Row ) : 0;
                ...
                expandInfo.size = m_Direction == e_Direction.Vertical ? (m_CellObjectHeight + m_SpacingY) * Mathf.CeilToInt((float)count / m_Row) : (m_CellObjectWidth + m_SpacingX) * Mathf.CeilToInt((float)count / m_Row); //计算 需展开的尺寸
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if(m_Direction == e_Direction.Vertical)
                    {
                        pos = m_CellObjectHeight * Mathf.FloorToInt( i / m_Row ) + m_SpacingY * ( Mathf.FloorToInt(i / m_Row) + 1 );
                        pos += (m_ExpandButtonHeight + m_SpacingY) * (k + 1);
                        pos += (m_CellObjectHeight + m_SpacingY) * Mathf.CeilToInt((float)beforeCellCount /m_Row);
                        rowPos = m_CellObjectWidth * ( i % m_Row ) + m_SpacingX * ( i % m_Row);
                        cellInfo.pos = new Vector3(rowPos, -pos, 0);
                    }
                    else
                    {
                        pos = m_CellObjectWidth * Mathf.FloorToInt(i / m_Row) + m_SpacingX * ( Mathf.FloorToInt(i / m_Row) + 1 );
                        pos += (m_ExpandButtonWidth + m_SpacingX) * (k + 1);
                        pos += (m_CellObjectHeight + m_SpacingY) * Mathf.CeilToInt( (float)beforeCellCount /m_Row );
            ...
            if (!m_IsInited)
            {
                ...
                if(m_Direction == e_Direction.Vertical)
                {
                    float contentSize = m_IsExpand ? (m_SpacingY + m_CellObjectHeight) * Mathf.CeilToInt((float)totalCount / m_Row) : 0;
                    contentSize += (m_SpacingY + m_ExpandButtonHeight) * buttonCount;
                    m_ContentRectTrans.sizeDelta = new Vector2(m_ContentRectTrans.sizeDelta.x, contentSize);
                }
                else
                {
                    float contentSize = m_IsExpand ? (m_SpacingX + m_CellObjectWidth) * Mathf.CeilToInt((float)totalCount / m_Row) : 0;
                    contentSize += (m_SpacingX + m_ExpandButtonWidth) * buttonCount;
        ...
        public override void OnClickExpand(int index)
        {
            ...
            for (int k = 0, length = m_ExpandInfos.Length; k < length; k++)
            {
                ...
                if (k >= index)
                {
                    ...
                    {
                        pos = m_ExpandButtonWidth * k + m_SpacingX * (k + 1);
                        pos += (m_CellObjectWidth + m_SpacingX) * Mathf.CeilToInt((float)beforeCellCount / m_Row);
                        button.transform.GetComponent<RectTransform>().anchoredPosition = new Vector3(pos, m_ExpandButtonY, 0);
                    }
