...
namespace CircularScrollView
{
    public class ExpandTipsCircularScrollView : UICircularScrollView
    {
        ...
        public override void OnClickCell(GameObject cell)
        {
            ...
            m_ExpandTips.transform.localPosition = new Vector3(0, -((m_SpacingY + m_CellObjectHeight) * curRow + m_TipsSpacing), 0);
            ...
            for(int i = 0, length = m_CellInfos.Length ; i < length; i++)
            {
                ...
                pos += (i/m_Row >= curRow && m_IsExpand) ? m_ExpandTipsHeight + m_TipsSpacing*2 - m_SpacingY : 0; //往下移 Tips框高 和 距离
